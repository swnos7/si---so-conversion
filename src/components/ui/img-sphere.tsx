...
const SphereImageGrid: React.FC<SphereImageGridProps> = ({
  ...
  const maxVisibleImages = useMemo(() => Math.max(1, images.length), [images.length]);
  const useInternalModal = !onSelectImage;
  ...
  const generateSpherePositions = useCallback((): SphericalPosition[] => {
    ...
    const imageCount = maxVisibleImages;
    ...
    return positions;
  }, [actualSphereRadius, maxVisibleImages]);
  ...
  }, [autoRotate, autoRotateSpeed, clampRotationSpeed, isDragging, momentumDecay, velocity]);
  ...
  useEffect(() => {
    if (!useInternalModal || !selectedImage) return undefined;
  ...
  }, [selectedImage, useInternalModal]);

  useEffect(() => {
    setImagePositions(generateSpherePositions());
  }, [generateSpherePositions, images]);
  ...
  }, [handleMouseMove, handleMouseUp, handleTouchEnd, handleTouchMove, isMounted]);
  ...
  const renderImageNode = useCallback(
    ...
    [baseImageSize, containerSize, handleImageSelection, hoverScale, hoveredIndex, worldPositions],
  );

  const renderSpotlightModal = () => {
    if (!useInternalModal || !selectedImage) return null;
        ...
        >
          <div className="relative max-h-[80vh]">
            <img
              ...
              className="h-full w-full rounded-t-2xl object-contain bg-black"
            />
            <button
              onClick={() => setSelectedImage(null)}
              className="absolute right-4 top-4 flex h-9 w-9 cursor-pointer items-center justify-center rounded-full bg-black/70 text-white transition-all hover:bg-black"
            >
              <X size={16} />
            </button>
          </div>
